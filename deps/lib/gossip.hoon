  ...
  ++  agent
    ...
    ++  on-load
      ...
      |^  =+  !<([%gossip old=state-any] (slot 2 ole))
          ...
          ::
          ::  edited from original: a version 2 state saved after the
          ::  upgrade already holds an h135 vase, so only migrate if not.
          ::
          =?  ile  ?=(%2 -.old)
            =/  new=(unit vase)  (mole |.(!<(vase ile)))
            ?^  new  u.new
            (next-vase:h136 !<(vase:h136 ile))
